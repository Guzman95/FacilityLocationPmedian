...
namespace BasedOnHarmony.Metaheuristicas
{
    class Utils
    {
        ...
        /// <summary>
        /// Obtieneel mejor de una poblacion
        /// </summary>
        /// <param name="myRandom"></param>
        /// <param name="numVertices"></param>
        /// <returns name="dimencionesN"></returns>
        public static Solution Getbest(List<Solution> Population)
        {
            Population.Sort((x, y) => x.Fitness.CompareTo(y.Fitness));
            return new Solution(Population[0]);
        }
        /// <summary>
        ...
        public static Solution LocalSearch(Solution solution, int valor) {
            ...
            for (var k = 0; k < valor; k++)
            {
                ...
                var b = NearestNeighborON(unviableNeighborsOn, solution, a);// Vecino mas cercano  controlado para no repetir
        ...
        //CON CONOCIMIENTO ABRIR
        public static int NearestNeighborON(List<int> unviableNeighborsOn, Solution solution, int a)
        {
        ...
        public static Solution LocalSearchGen(Solution solution, Solution best)
        {
            ...
            for (var i = 0; i < solution.MyAlgorithm.MyProblem.NumVertices; i++)
            {
                if (newsolution.Vertices[i] != best.Vertices[i] && best.Vertices[i] == 1)
                {
                    newsolution.Activar(i);
                    listSecondGen.Add(i);
                }
            }
        ...
        public static Solution RepairOperatorLocalSearch(Solution newsolution, List<int> listSecondGen)
        {
            Random rnd = new Random();
            while (newsolution.PosInstalaciones.Count > newsolution.MyAlgorithm.MyProblem.PMedianas)
            {
                int posSeconfGen = listSecondGen[rnd.Next(listSecondGen.Count)];
                newsolution.InActivar(posSeconfGen);
